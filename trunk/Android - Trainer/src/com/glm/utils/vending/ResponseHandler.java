...
public class ResponseHandler {
    ...
    public static void purchaseResponse(
            final Context context, final com.glm.utils.vending.Consts.PurchaseState purchaseState, final String productId,
        ...
        new Thread(new Runnable() {
            public void run() {
            	try{
                	...
                	oDataBase.execSQL("INSERT INTO STORE_ORDERS (store_id, state, product_id, product_desc,order_time) VALUES ('"+orderId+"','"+purchaseState+"','"+productId+"','','"+purchaseTime+"')");
                	oDataBase.close();
                	oDB.close();   
                	oDataBase=null;
                	oDB=null;
                	if(productId.compareToIgnoreCase("polarcardio")==0){
                		SharedPreferences oPrefs = context.getSharedPreferences("aTrainer",Context.MODE_PRIVATE);
					...
					Log.e(this.getClass().getCanonicalName(),"Error null point order");
				}catch (SQLException e) {
					Log.e(this.getClass().getCanonicalName(),"Error sql order");
				}
